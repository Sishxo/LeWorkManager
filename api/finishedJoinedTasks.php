<?php
	...
	else
	{
		...
		$sql = "SELECT * FROM `platform_project` WHERE ((adminCheck = 1) and (realEndFlag = 1) and (proName LIKE '%".
			$finishedCondition."%') and (id IN (".$sqlInSelect."))) ORDER BY createTime DESC LIMIT ".$startNum.", ".$length;
		// echo $sql;
		...
		if ($result)
		{
            ...
            $sqlNew = "SELECT COUNT(*) AS total FROM `platform_project` WHERE ((adminCheck = 1) and (realEndFlag = 1) and (proName LIKE '%"
			    .$finishedCondition."%') and (id IN (".$sqlInSelect.")))";
