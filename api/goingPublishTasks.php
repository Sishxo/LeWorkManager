<?php
	...
	{
		$sql = "SELECT * FROM `platform_project` WHERE ((headPerson = '".$headPerson."') AND (realEndFlag = 0)"
			." AND (proName LIKE '%".$goingCondition."%')) ORDER BY createTime DESC LIMIT ".$startNum.", ".$length;
		// echo $sql;
		...
		if ($result)
		{
			...
			$sqlNew = "SELECT COUNT(*) AS total FROM `platform_project` WHERE ((headPerson = '".$headPerson
				."') AND (realEndFlag = 0) AND (proName LIKE '%".$goingCondition."%'))";
			// echo $sqlNew;
