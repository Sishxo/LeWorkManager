<?php
    ...
    $page = $_POST['page'];
    $length = $_POST['length'];
	$signUpCondition = $_POST['signUpCondition'];
	...
	else
	{
		...
		$sql = "SELECT * FROM `platform_project` WHERE ((adminCheck = 1) and (signUpEndTime <> '0000-00-00 00:00:00') and ((signUpEndTime >= '".$nowDateTime.
			"') OR (id NOT IN (".$sqlInSelect."))) and proName LIKE '%".$signUpCondition."%' and realEndFlag = 0 and projectType <> 9 and projectType <> 10 ) ORDER BY createTime DESC LIMIT ".$startNum.", ".$length;
		// echo $sql;
		...
		if ($result)
		{
			...
			$sqlNew = "SELECT COUNT(*) AS total FROM `platform_project` WHERE ((adminCheck = 1) and (signUpEndTime <> '0000-00-00 00:00:00') and ((signUpEndTime >= '".$nowDateTime.
				"') OR (id NOT IN (".$sqlInSelect."))) and proName LIKE '%".$signUpCondition."%' and realEndFlag = 0)";
			// $sqlNew = "SELECT COUNT(*) AS total FROM `platform_project` WHERE ((adminCheck = 1) and (signUpEndTime <> '0000-00-00 00:00:00') and (signUpEndTime >= '".$nowDateTime.
