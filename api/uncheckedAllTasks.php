<?php
	...
	else
	{
        ...
        // echo $nowDateTime;
		$sql = "SELECT * FROM `platform_project` WHERE ((adminCheck = 0) and ((taskMoney > 0) or (taskPoint > 0)) AND (proName LIKE '%"
		...
		if ($result)
		{
			...
			$sqlNew = "SELECT COUNT(*) AS total FROM `platform_project` WHERE ((adminCheck = 0) and ((taskMoney > 0) or (taskPoint > 0)) AND (proName LIKE '%"
